<?php
        class StatuAction extends Action{
		public function readList(){
			$Statu = M('Statu'); // 实例化Data数据对象
     			import('ORG.Util.Page');// 导入分页类
 		    	$count      = $Statu->count();// 查询满足要求的总记录数
     			$Page       = new Page($count,20);// 实例化分页类 传入总记录数
     			$show       = $Page->show();// 分页显示输出
     			// 进行分页数据查询
     			$list = $Statu->order('statu_id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
			for($i=0;$i<20;$i++){
				if($list[$i]['result']=='Accepted!')
					...
					$list[$i]['result']='<font color="#FF0000">'.$list[$i]['result'].'</font>';
                                        ...
                                        $list[$i]['result']="<a href=__URL__/readError/id/".$list[$i]['statu_id']."><font color='blue'>".$list[$i]['result'].'</font></a>';
			}
     			$this->assign('list',$list);// 赋值数据集
     			$this->assign('page',$show);// 赋值分页输出
     			$this->display(); // 输出模板
		}
		public function readError($id=100000000){
			$errorLog = file_get_contents("goj/Common/errorLog/$id.log");
